...
namespace Lagdo\Monolog\Formatter;
...
class SyslogFormatter extends LineFormatter
{
    ...
    private function formatExtra(array $record): array
    {
        ...
        $extra['priority'] = self::calculatePriority($record);
        $extra['structured-data'] = self::NILVALUE;

        return $extra;
    }

    private static function calculatePriority(array $record): int
    {
        return (self::SYSLOG_FACILITY_USER * 8) + match(strtolower($record['level_name'])) {
            LogLevel::DEBUG => 7,
            LogLevel::INFO => 6,
            LogLevel::NOTICE => 5,
            LogLevel::WARNING => 4,
